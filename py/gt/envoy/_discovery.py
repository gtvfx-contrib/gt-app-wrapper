"""
Package discovery for wrapper environments.

Supports two methods of discovering packages:
1. Auto-discovery: Search directories specified in ENVOY_PKG_ROOTS for git repositories
2. Config file: Explicit list of package paths
"""

import os
import logging
from pathlib import Path
from typing import List, Optional, Dict
import json

from ._exceptions import WrapperError


logger = logging.getLogger(__name__)


class PackageInfo:
    """Information about a discovered package."""
    
    def __init__(self, root: Path, name: str):
        """
        Initialize package information.
        
        Args:
            root: Root directory of the package
            name: Name of the package (directory name)
        """
        self.root = root
        self.name = name
        self.envoy_env = root / "envoy_env"
        
    def __repr__(self):
        return f"PackageInfo(name={self.name}, root={self.root})"
    
    def __str__(self):
        return f"{self.name} ({self.root})"


def is_git_repo(path: Path) -> bool:
    """
    Check if a directory is a git repository.
    
    Args:
        path: Path to check
        
    Returns:
        True if path contains a .git directory
    """
    return (path / ".git").is_dir()


def has_envoy_env(path: Path) -> bool:
    """
    Check if a directory has an envoy_env subdirectory.
    
    Args:
        path: Path to check
        
    Returns:
        True if path contains an envoy_env directory
    """
    return (path / "envoy_env").is_dir()


def validate_package(path: Path) -> bool:
    """
    Validate that a path is a valid envoy package.
    
    A valid package must:
    - Be a directory
    - Have an envoy_env subdirectory
    
    Args:
        path: Path to validate
        
    Returns:
        True if path is a valid package
    """
    if not path.is_dir():
        return False
    
    if not has_envoy_env(path):
        return False
    
    return True


def find_git_repos(root_dir: Path, max_depth: int = 5) -> List[Path]:
    """
    Recursively find git repositories under a root directory.
    
    Args:
        root_dir: Root directory to search
        max_depth: Maximum depth to search
        
    Returns:
        List of paths to git repository roots
    """
    repos = []
    
    if not root_dir.is_dir():
        logger.warning(f"Root directory does not exist: {root_dir}")
        return repos
    
    def search_dir(path: Path, depth: int = 0):
        """Recursively search for git repos."""
        if depth > max_depth:
            return
        
        try:
            # Check if this directory is a git repo
            if is_git_repo(path):
                repos.append(path)
                # Don't search inside git repos
                return
            
            # Search subdirectories
            for item in path.iterdir():
                if item.is_dir() and not item.name.startswith('.'):
                    search_dir(item, depth + 1)
        except PermissionError:
            logger.debug(f"Permission denied: {path}")
        except Exception as e:
            logger.debug(f"Error searching {path}: {e}")
    
    search_dir(root_dir)
    return repos


def discover_packages_from_roots(root_dirs: List[str]) -> List[PackageInfo]:
    """
    Discover packages in specified root directories.
    
    Searches for git repositories and validates them as envoy packages.
    
    Args:
        root_dirs: List of root directory paths
        
    Returns:
        List of discovered packages
    """
    packages = []
    
    for root_str in root_dirs:
        root = Path(root_str).resolve()
        logger.debug(f"Searching for packages in: {root}")
        
        # Find all git repos under this root
        git_repos = find_git_repos(root)
        logger.debug(f"Found {len(git_repos)} git repositories in {root}")
        
        # Validate each repo as a package
        for repo_path in git_repos:
            if validate_package(repo_path):
                package = PackageInfo(
                    root=repo_path,
                    name=repo_path.name
                )
                packages.append(package)
                logger.info(f"Discovered package: {package}")
            else:
                logger.debug(f"Git repo is not an envoy package: {repo_path}")
    
    return packages


def discover_packages_auto() -> List[PackageInfo]:
    """
    Auto-discover packages using ENVOY_PKG_ROOTS environment variable.
    
    ENVOY_PKG_ROOTS should contain a list of root directories separated by
    the OS path separator (';' on Windows, ':' on Unix).
    
    Returns:
        List of discovered packages
    """
    roots_str = os.environ.get('ENVOY_PKG_ROOTS', '')
    
    if not roots_str:
        logger.debug("ENVOY_PKG_ROOTS not set, no auto-discovery")
        return []
    
    # Split by OS path separator
    separator = ';' if os.name == 'nt' else ':'
    root_dirs = [r.strip() for r in roots_str.split(separator) if r.strip()]
    
    if not root_dirs:
        logger.debug("ENVOY_PKG_ROOTS is empty")
        return []
    
    logger.info(f"Auto-discovering packages from {len(root_dirs)} root(s)")
    return discover_packages_from_roots(root_dirs)


def load_packages_from_config(config_file: Path) -> List[PackageInfo]:
    """
    Load package paths from a configuration file.
    
    Config file format (JSON):
    {
        "packages": [
            "/path/to/package1",
            "/path/to/package2"
        ]
    }
    
    or (JSON array):
    [
        "/path/to/package1",
        "/path/to/package2"
    ]
    
    Args:
        config_file: Path to configuration file
        
    Returns:
        List of packages from config file
        
    Raises:
        WrapperError: If config file is invalid
    """
    if not config_file.is_file():
        raise WrapperError(f"Config file not found: {config_file}")
    
    try:
        with open(config_file, 'r') as f:
            data = json.load(f)
    except json.JSONDecodeError as e:
        raise WrapperError(f"Invalid JSON in config file: {e}")
    except Exception as e:
        raise WrapperError(f"Error reading config file: {e}")
    
    # Support both {"packages": [...]} and direct array [...]
    if isinstance(data, dict):
        package_paths = data.get('packages', [])
    elif isinstance(data, list):
        package_paths = data
    else:
        raise WrapperError("Config file must be a JSON object or array")
    
    packages = []
    for path_str in package_paths:
        path = Path(path_str).resolve()
        
        if not validate_package(path):
            logger.warning(f"Invalid package in config: {path}")
            continue
        
        package = PackageInfo(
            root=path,
            name=path.name
        )
        packages.append(package)
        logger.info(f"Loaded package from config: {package}")
    
    return packages


def get_packages(config_file: Optional[Path] = None) -> List[PackageInfo]:
    """
    Get all packages using config file or auto-discovery.
    
    If config_file is provided, only packages from the config are used.
    Otherwise, auto-discovery is attempted using ENVOY_PKG_ROOTS.
    
    Args:
        config_file: Optional path to config file
        
    Returns:
        List of discovered packages
    """
    if config_file:
        logger.info(f"Using package config file: {config_file}")
        return load_packages_from_config(config_file)
    else:
        logger.debug("No config file, attempting auto-discovery")
        return discover_packages_auto()


def get_package_env_files(packages: List[PackageInfo]) -> Dict[str, List[Path]]:
    """
    Get all environment files from discovered packages.
    
    Returns a mapping of package names to their environment JSON files.
    
    Args:
        packages: List of packages to scan
        
    Returns:
        Dict mapping package name to list of environment file paths
    """
    env_files = {}
    
    for package in packages:
        files = []
        wrapper_env = package.envoy_env
        
        if wrapper_env.is_dir():
            # Find all .json files in envoy_env
            for json_file in wrapper_env.glob("*.json"):
                # Skip commands.json as it's handled separately
                if json_file.name != "commands.json":
                    files.append(json_file)
        
        if files:
            env_files[package.name] = files
            logger.debug(f"Package {package.name}: {len(files)} environment file(s)")
    
    return env_files


def get_package_commands_files(packages: List[PackageInfo]) -> Dict[str, Path]:
    """
    Get commands.json files from discovered packages.
    
    Returns a mapping of package names to their commands.json files.
    
    Args:
        packages: List of packages to scan
        
    Returns:
        Dict mapping package name to commands.json path
    """
    commands_files = {}
    
    for package in packages:
        commands_file = package.envoy_env / "commands.json"
        
        if commands_file.is_file():
            commands_files[package.name] = commands_file
            logger.debug(f"Package {package.name}: has commands.json")
    
    return commands_files
